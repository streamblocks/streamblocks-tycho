...
public interface Convert {
	...
	default Expression apply(ValueLambda value) {
		return new ExprLambda(value.parameters().stream().map(param -> new ParameterVarDecl(param.type(), param.name(), apply(param.defaultValue().orElse(ValueUndefined.undefined())))).collect(Collectors.toList()), value.body(), value.type());
	}
