...
import se.lth.cs.tycho.transformation.cal2am.Knowledge;
...
public class CompositionController implements Controller {
	...
	public class CompositionState implements State {
		...
		private Test convert(int actor, Test test) {
			...
			return new Test(conditionIndex(actor, test.condition()), state(newStatesTrue, tokens), state(newStatesFalse, tokens), test.getOrderNumber());
		}
