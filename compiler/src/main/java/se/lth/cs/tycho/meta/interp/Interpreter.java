...
public interface Interpreter {
	...
	default Value eval(ExprIndexer expr, Environment env) {
		...
		if (value instanceof ValueList
				&& index instanceof ValueInteger
				&& ((ValueInteger) index).integer() > -1
				&& ((ValueInteger) index).integer() < ((ValueList) value).elements().size()) {
			return ((ValueList) value).elements().get(((ValueInteger) index).integer());
		}
		return ValueUndefined.undefined();
	}
	...
	default Value eval(ExprLiteral expr, Environment env) {
		switch (expr.getKind()) {
			...
			case Integer:
				return expr.asInt().isPresent() ? new ValueInteger(expr.asInt().getAsInt()) : ValueUndefined.undefined();
			case Char:
