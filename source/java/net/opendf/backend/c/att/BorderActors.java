...
public class BorderActors extends Module<BorderActors.Required> {
	...
	public void inputActor(Connection conn, PrintWriter writer) {
		...
		String type = get().ctype(conn).plainType();
		...
		writer.println("	size_t length = start < head"+name+" ? head"+name+" - start : size"+name+" - start;");
		writer.println("	size_t count = fread(&buffer"+name+"[start], sizeof("+type+"), length, input_file"+name+");");
	...
	public void outputActor(Connection conn, PrintWriter writer) {
		...
		writer.println("	size_t count = fwrite(&buffer"+name+"[head"+name+"], sizeof("+type+"), length, output_file"+name+");");
		writer.println("	tokens"+name+" -= count;");
		writer.println("	if (count < length) {");
		writer.println("		fprintf(stderr, \"IO error\\n\");");
		writer.println("		exit(1);");
	...
	public void borderActors(Network net, PrintWriter writer) {
		for (Connection conn : net.getConnections()) {
			if (conn.getSrcNodeId() == null) {
				get().inputActor(conn, writer);
			}
			if (conn.getDstNodeId() == null) {
				get().outputActor(conn, writer);
			}
		}
	}
	...
	public List<String> borderActorNames(Network net) {
		...
		for (Connection conn : net.getConnections()) {
			if (conn.getSrcNodeId() == null) {
				names.add("input_to" + get().bufferName(conn));
			}
			if (conn.getDstNodeId() == null) {
				names.add("output_from" + get().bufferName(conn));
			}
		}
		return names;
	}
