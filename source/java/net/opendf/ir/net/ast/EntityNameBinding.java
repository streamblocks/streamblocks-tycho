...
public class EntityNameBinding implements StructureStmtVisitor<Object, java.util.Map.Entry<String,EntityExpr>[]>{

	public EntityNameBinding(NetworkDefinition network){
		java.util.Map.Entry<String,EntityExpr>[] names = network.getEntities();
		for(StructureStatement s : network.getStructure()){
			s.accept(this, names);
		}
	}

	public Object visitStructureConnectionStmt(StructureConnectionStmt stmt, Entry<String, EntityExpr>[] names) {
		lookupEntity(stmt.getSrc(), names);
		lookupEntity(stmt.getDst(), names);
		return null;
	}

	public Object visitStructureIfStmt(StructureIfStmt stmt, Entry<String, EntityExpr>[] names) {
	...
	public Object visitStructureForeachStmt(StructureForeachStmt stmt, Entry<String, EntityExpr>[] names) {
		for(StructureStatement s : stmt.getStatements()){
			s.accept(this, names);
		}
		return null;
	}
	private void lookupEntity(PortReference ref, Entry<String, EntityExpr>[] names){
		...
		if(entityName != null){
			...
			if(entity == null){
				...
				System.err.println("bound: " + entityName);				
			}
		} else{
			//local port
		}
